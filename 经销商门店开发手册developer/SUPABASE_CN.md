...
**快捷方式（推荐）**：打开仓库里的 **`/developer/distributor.sql`**，复制**全部内容**到 SQL Editor 执行**一次**，即等价于依次执行 `001_init.sql` + `002_dealer_contacts.sql`（与 `supabase/migrations/` 目录中的迁移保持内容一致，便于白标/经销商一键建库）。
...
6. 再次新建 query, 粘贴 `/supabase/migrations/002_dealer_contacts.sql` 的全部内容并执行
   - 此脚本创建 `dealer_contacts` (联系人) 和 `dealer_ledger` (账本) 两张表
...
| `server` | `/server/contacts` | GET | 获取经销商的农户联系人列表 (需JWT) |
| `server` | `/server/contacts` | POST | 添加/更新农户联系人 (需JWT) |
| `server` | `/server/contacts/:id` | PATCH | 更新联系人 (静音、改名等) |
| `server` | `/server/contacts/:id` | DELETE | 删除联系人 |
| `server` | `/server/ledger` | GET | 获取经销商账本数据 (需JWT) |
| `server` | `/server/ledger` | POST | 保存经销商账本数据 (需JWT, 乐观锁) |
...
>
> **ledger GET 响应体**: `{ "data": {...LedgerState}, "version": 3, "updatedAt": "..." }`
>
> **ledger POST 请求体**: `{ "data": {...LedgerState}, "version": 3 }`
> - 返回 200: `{ "success": true, "version": 4 }`
> - 版本冲突 409: `{ "success": false, "conflict": true, "serverVersion": 5 }`
...
| `dealer_contacts` | `id` (UUID, 自动) | 经销商农户联系人 | service_role only |
| `dealer_ledger` | `dealer_id` (UUID → auth.users) | 经销商账本 (单行 JSONB) | service_role only |
