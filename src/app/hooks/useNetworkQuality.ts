...
/** 根据网络状态优化 Unsplash 图片 URL */
export function optimizeImageUrl(url: string, quality: NetworkQuality): string {
  if (!url) return url;
  
  // 如果是 Unsplash URL，替换 w 和 q 参数
  if (url.includes('images.unsplash.com') || url.includes('plus.unsplash.com')) {
    let optimized = url;

    // 用捕获组保留原始 ? 或 & 前缀，避免把 ?w= 错误替换成 &w= 导致 URL 损坏
    if (/[&?]w=\d+/.test(optimized)) {
      optimized = optimized.replace(/([&?])w=\d+/, `$1w=${quality.imageWidth}`);
    } else {
      optimized += (optimized.includes('?') ? '&' : '?') + `w=${quality.imageWidth}`;
    }

    if (/[&?]q=\d+/.test(optimized)) {
      optimized = optimized.replace(/([&?])q=\d+/, `$1q=${quality.imageQuality}`);
    } else {
      optimized += `&q=${quality.imageQuality}`;
    }

    // 添加 WebP 格式
    if (!optimized.includes('fm=webp')) {
      optimized = optimized.replace(/([&?])fm=[^&]+/, `$1fm=webp`);
      if (!optimized.includes('fm=webp')) {
        optimized += '&fm=webp';
      }
    }
  ...
  if (url.includes('placehold.co')) {
    return url.replace(/\/\d+x\d+\//, `/${Math.min(quality.imageWidth, 200)}x${Math.min(quality.imageWidth, 200)}/`);
  }
