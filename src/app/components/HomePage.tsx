...
const MarketCatalogOverlay = lazy(() => import("./MarketCatalogOverlay").then((m) => ({ default: m.MarketCatalogOverlay })));
...
export function HomePage() {
  ...
  const optimizedBannerUrls = useMemo(() => 
    bannerImages.map(img => ({
      ...img,
      url: getThumbnailUrl(img.url, "bannerCarousel"),
    })),
    [bannerImages]
  );
  ...
  const liveThumbnailUrl = useMemo(() => {
    ...
    if (coverUrl) return getThumbnailUrl(coverUrl, "liveCover");
    const thumb = config.liveStreams?.[0]?.thumbnail;
    if (thumb) return getThumbnailUrl(thumb, "liveCover");
  ...
  return (
    <div ref={homeRootRef} className="min-h-screen" style={{ backgroundColor: 'var(--app-bg)' }}>
      ...
      {currentView.type === "marketCatalog" && (
        <Suspense fallback={<div className="w-full h-full bg-gray-100 animate-pulse"></div>}>
          <MarketCatalogOverlay onClose={() => setCurrentView({ type: "home" })} />
        </Suspense>
      )}
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {searchResults.products.map((product: any) => (
                          ...
                          >
                            <LazyImage
                              src={getThumbnailUrl(product.image, "productSearch")}
                              alt={product.name}
                          ...
                          {searchResults.articles.map((article: any) => (
                            ...
                            >
                              <div className="flex items-center gap-3">
                                {article.thumbnail && (
                                  <LazyImage
                                    src={getThumbnailUrl(article.thumbnail, "articleCard")}
                                    alt={article.title}
            ...
            >
              <BannerCarousel {...sliderSettings}>
                {optimizedBannerUrls.map((image, index) => (
                  <div 
                    key={image.id} 
                    className="slider-item relative h-full min-h-0 w-full"
                    onClick={() => {
                  ...
                  >
                    <img
                      src={image.url}
                      alt={image.alt}
                      className="absolute inset-0 block h-full w-full max-w-none object-fill object-center"
              ...
              >
                <div className="flex-1 min-h-0 w-full flex items-center justify-center overflow-hidden">
                  {config.homeIcons?.aiAssistantIconUrl ? (
                    <img src={config.homeIcons.aiAssistantIconUrl} alt={config.homeIcons?.aiAssistantLabel || t.home.aiAssistant} className="max-w-full max-h-full w-auto h-auto object-contain object-center" />
                  ) : (
              ...
              >
                <div className="flex-1 min-h-0 w-full flex items-center justify-center overflow-hidden">
                  {config.homeIcons?.statementIconUrl ? (
                    <img src={config.homeIcons.statementIconUrl} alt={config.homeIcons?.statementLabel || t.home.statement} className="max-w-full max-h-full w-auto h-auto object-contain object-center" />
                  ) : (
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      {article.thumbnail ? (
                        <LazyImage 
                          src={getThumbnailUrl(article.thumbnail, "articleCard")}
                          alt={article.title}
                          className="w-16 h-16 sm:w-20 sm:h-20 bg-gray-100 rounded-xl flex-shrink-0"
                        />
                      ) : (
